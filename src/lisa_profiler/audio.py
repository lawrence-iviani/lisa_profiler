...
def read_file_and_play(filename, audio_params): #, data_queue, event):
	...
	def play_callback(outdata, frames, time, status):
		...
		if len(data) < len(outdata):
			...
			zero_pad = np.zeros( dtype='float32', shape=(len(outdata) - len(data), 1) )

			outdata[len(data):] = zero_pad
	...
	with sf.SoundFile(filename) as f:
		...
		# _print_debug(function='read_file_and_play', msg="file length: {} samples - {} sec. - expected bytes per buffer {} ".format(
		# 			f.frames,
		# 			_duration_file,
		# 			audio_params.blocksize*4))
		for _n in range(audio_params.buffersize):
			data = f.read(audio_params.blocksize, dtype='float32')
			if  len(data) == 0:
				break
			if len(data) < audio_params.blocksize:
				zero_pad = np.zeros(audio_params.blocksize - len(data) , dtype='float32')
				data = np.append(data , zero_pad)

			# if not data:
			# 	_print_debug(function='read_file_and_play', msg="break no data to prefill")
			# 	break
			#_print_debug(function='read_file_and_play', msg="Prefill queue[{}] buffer_{} with {} len={}".format(data_queue, _n, data.__class__, len(data[:])))
		...
		# if there is space, better fill an empty buffer of 0s
		if data_queue.qsize() < audio_params.buffersize:
			data_queue.put_nowait(np.zeros(audio_params.blocksize , dtype='float32'))
		...
		stream = sd.OutputStream(samplerate=f.samplerate, blocksize=audio_params.blocksize,
